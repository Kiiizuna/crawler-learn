...
const movieFromDiv = function(div) {
    ...
    // .text() 获取文本信息
    movie.name = e('.movie-name-text').text()
    movie.score = e('.rating_num').text()
    movie.info = e('.movie-misc').text()
    movie.ranking = e('.rank-num').text()
    const picFather = e('.movie-content')
    // movie.ranking = pic.find('em').text()
    // 元素的属性用 .attr('属性名') 确定
    movie.coverUrl = picFather.find('img').attr('src')
    // find('标签名')
    return movie
}
...
const moviesFromUrl = function(url) {
    ...
    request(url, function(error, response, body) {
        ...
        if (error === null && response.statusCode == 200) {
            ...
            const movieDivs = e('.movie-content')
            log('movieDivs is',movieDivs)
            ...
            for(let i = 0; i < movieDivs.length; i++) {
                ...
                movies.push(23232323)
                log('movies isis', movies)
            }
            // 保存 movies 数组到文件中
            saveMovies(movies)
...
const __main = function() {
    // 这是主函数
    // 下载网页, 解析出电影信息, 保存到文件
    const url = 'https://movie.douban.com/typerank?type_name=%E6%82%AC%E7%96%91&type=10&interval_id=100:90&action='
    moviesFromUrl(url)
}
